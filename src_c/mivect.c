  ...
  if (!v || v->vec_mag != MI_VECTEUR_NMAGIQ)
    {
      ...
      if (!v)
        MI_FATALPRINTF("memoire pleine pour vecteur de %d (%s)", nouvtail, strerror(errno));
  ...
  else if (v->vec_compte + nb >= v->vec_taille)
    {
      ...
      if (!nouvec)
        MI_FATALPRINTF("vecteur trop grand de taille %d (%s)", nouvtail, strerror(errno));
...
struct Mi_trouve_st mi_vecteur_comp(struct Mi_Vecteur_st*v, int rang)
{
  ...
  if (!v || v->vec_mag != MI_VECTEUR_NMAGIQ) return r;
...
{
  if (!v || v->vec_mag != MI_VECTEUR_NMAGIQ) return;
  unsigned cnt = v->vec_compte;
  assert (cnt <= v->vec_taille);
  if (rang<0) rang += (int)cnt;
  if (rang>=0 && rang<(int)cnt)
...
{
  if (!v || v->vec_mag != MI_VECTEUR_NMAGIQ) return 0;
  return v->vec_compte;
}
